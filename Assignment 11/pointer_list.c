...
typedef void* (*CopyFun)(void* value);

void* copy_string(void* value) {
    char* src = (char*)value;
    char* dst = malloc(strlen(src) + 1);
    if (dst == NULL) return NULL;
    strcpy(dst, src);
    return dst;    // Or just return strdup((char*)value); and thats it
}

void* copy_int(void* value) {  // cuz of void* we need a deep copy.
